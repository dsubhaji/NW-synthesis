...
public class ProbabilisticNetworkModeler {
			...
			int peerLevel, double q, double randomAffiliationProbability, Vector<String> vertexIDs)
	{
		...
		if(peerLevel < height)
		{
			...
			//int counter=(new Double(maxNoOfLinksBetnVerticesAtThisLevel)).intValue();
			while(counter>0)
			{
	...
	public Vector<AffiliationDataBean> generateAffiliations_level(Vector<Vector<Integer>> levels, int branch, int height, double p, 
			int peerLevel, double q, double randomAffiliationProbability, Vector<String> vertexIDs)
	{
		Vector<AffiliationDataBean> affiliations = new Vector<AffiliationDataBean>();
		Vector <Hashtable <Integer,Vector<Integer>>> leafNodesForParents = new Vector<Hashtable<Integer,Vector<Integer>>>();
		...
		if(peerLevel < height)
		{
			...
			//System.out.println("Before Values of q "+prob);
			double actualNoOfLinksBetnVerticesAtThisLevel = Math.rint(prob*maxNoOfLinksBetnVerticesAtThisLevel);
