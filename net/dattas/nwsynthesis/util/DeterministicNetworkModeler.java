...
public class DeterministicNetworkModeler {
	...
	public static void main(String[] args) {
		...
		DeterministicNetworkModeler dnm = new DeterministicNetworkModeler();
		Vector<String> vertexIDs = dnm.generateVertices(height, branch);
		System.out.println("Vertices " + vertexIDs +"\n");
		
		Vector<Vector<Integer>> levels = dnm.generateLevels(height, branch);
		System.out.println("No of levels " + levels.size());
		System.out.println("levels " + levels + "\n");
		
		Vector<AffiliationDataBean> affiliations = dnm.generateAffiliations(levels, branch, height, p, 0, 0.0, 0.0, vertexIDs);
	...
	public Vector<Vector<Integer>> generateLevels(int height, int branch)
	{
		...
		//System.out.println(levelWidths);
	...
	public Vector<AffiliationDataBean> generateAffiliations(Vector<Vector<Integer>> levels, int branch, int height, double p, int peerLevel, double q, double randomAffiliationProbability, Vector<String> vertexIDs)
	{
		
		Vector<AffiliationDataBean> affiliations = new Vector<AffiliationDataBean>();
		
		// Data structure to hold leaf children for each corresponding parent 
		Vector <Hashtable <Integer,Vector<Integer>>> parentAndLeafChildren = new Vector<Hashtable<Integer,Vector<Integer>>>();
		
		// Data structure to hold children (other than leaf) for each corresponding parent 
		Vector <Hashtable <Integer,Vector<Integer>>> parentAndNonLeafChildren = new Vector<Hashtable<Integer,Vector<Integer>>>();
		
		// Data structure to hold all sets of parent and children (leaf and non-leaf)
		Vector <Hashtable <Integer,Vector<Integer>>> parentAndAllChildren = new Vector<Hashtable<Integer,Vector<Integer>>>();
		
		int noOfLevels = levels.size();
		
		// Start: Generate the usual tree affiliations and record leaves and non leaves under each parent
		...
		//	System.out.println("*Edges");
			...
			for(int j=0; j<noOfVerticesInCurrentLevel; j++)
			{
				...
				Vector<Integer> childrenForAParent = new Vector<Integer>();
				...
				//	System.out.println("Parent: " + currentAffiliationSource + " Child: " + currentAffliationDestination);
					if(currentLevelNo == (noOfLevels-2)) // detecting leaves
						{
							
							leavesForAParent.add(currentAffliationDestination);
							
						}
					else // detecting non leaf children
					{
						childrenForAParent.add(currentAffliationDestination);
					}
				...
				if(currentLevelNo == (noOfLevels-2)) // detecting leaves
				{
					Hashtable<Integer, Vector<Integer>> parentWithLeaves = new Hashtable<Integer, Vector<Integer>>();
					parentWithLeaves.put(currentAffiliationSource,leavesForAParent);
					parentAndLeafChildren.add(parentWithLeaves);
				}
				
				else // detecting non leaves
				{
					Hashtable<Integer, Vector<Integer>> parentWithNonLeaves = new Hashtable<Integer, Vector<Integer>>();
					parentWithNonLeaves.put(currentAffiliationSource,childrenForAParent);
					parentAndNonLeafChildren.add(parentWithNonLeaves);
				}
			}
		...
		parentAndAllChildren.addAll(parentAndNonLeafChildren);
		parentAndAllChildren.addAll(parentAndLeafChildren);
		
		
		System.out.println("Parent and non-leaf children: " + parentAndNonLeafChildren);
		
		System.out.println("Parent and leaf children: " + parentAndLeafChildren);
		
		System.out.println("Parent and all children: " + parentAndAllChildren);
		
		
		// End: Generate the usual tree affiliations and leaves and non leaves under each parent
		...
		for(int i=0; i<parentAndLeafChildren.size(); i++)
		{
			Hashtable<Integer, Vector<Integer>> temp = parentAndLeafChildren.elementAt(i);
	...
	// Method to generate affiliations by levels
	public Vector<AffiliationDataBean> generateAffiliationsByLevels(int branch, int height, Vector<String> vertexIDs, Vector<Vector<Integer>> levels, Vector <Hashtable <Integer,Vector<Integer>>> parentAndAllChildren, int levelNumber, double connectionProbabalityAtThisLevel, boolean connectOnlyUnderSameParent)
	{
		
		Vector<AffiliationDataBean> affiliations = new Vector<AffiliationDataBean>();
	
		/* Algorithm
		 * (Create a method for returning parents-children
		 * For the given level, find how many of the vertices are children under same parent
		 * Check if only children under same parent will connect, or all children at that level irrespective of parentage will connect
		 * Connect all of relevant children with one another 
		 * 
		 */
